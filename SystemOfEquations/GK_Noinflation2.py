...
class GK_NOINFLATION():
    ...
    def __init__(self):            
        ...
        self.variables         = { k:v['f'] for k,v in self.EQUATIONS.items() if v['type']=='differential' }
        self.intermediaryfuncs = { k:v['f'] for k,v in self.EQUATIONS.items() if v['type']=='intermediary' }  
        self.auxiliaryfuncs    = { k:v['f'] for k,v in self.EQUATIONS.items() if v['type']=='auxilliary'   } 
