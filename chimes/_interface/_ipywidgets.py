...
class Interface:
    ...
    def get_Loading(self):
        ...
        def valuechange(change):
            ...
            with LoadOut:
                ...
                display(self.hub.get_presets(returnas=True))
                ...
                display(HTML(self.hub.get_equations_description().to_html().replace("\\n","<br>")))
    ...
    def get_set(self):
        ...
        '''
        SetOut      = widgets.Output()
        def showSummary(obj):
            with SetOut:
                alldicts=self.hub.get_new_summary()
                display(Markdown('### General properties'))
                display(pd.DataFrame(alldicts['Field Basic Properties']))
                display(Markdown('### Parameters'))
                display(pd.DataFrame(alldicts['Parameters values']))
                display(Markdown('### Differential variables'))                
                display(pd.DataFrame(alldicts['Differential Variables values']))
                display(Markdown('### State variables'))                
                display(pd.DataFrame(alldicts['State Variables values']))
        def setvalue(obj):
            self.hub.set_dparam(**eval(SetDparamText.value))
            clickSET.button_style='success'
        def set1(obj): 
            print(eval(SetRegion.value))
            self.hub.set_dparam(**{'nr'  : eval(SetRegion.value)})
        def set2(obj): 
            print(eval(SetParrallel.value))
            self.hub.set_dparam(**{'nx'  : eval(SetParrallel.value)})
        def set3(obj): 
            self.hub.set_dparam(**{'Tmax': eval(SetTmax.value)})
        ...
        clickSETR.on_click(set1)    
        clickSETT.on_click(set3)   
        clickSETP.on_click(set2)   
        ...
        #########################################################
        text='You can set an arbitrary quantity of values using\
dictionnary writing. For example {"a":5,"b":0.33} will change the value of a and b'
        return widgets.VBox([
                             widgets.HTML(value='<strong>Set values</strong>'),
                             widgets.HTML(value=text),
                             widgets.HBox([SetDparamText,clickSET]),
                             widgets.HTML(value='<strong>Explore fields values</strong>'),
                             widgets.HBox([clickGUI,clear]),
                             SetOut,
