...
# _LTYPES = config.get_current('_LTYPES')
# _LTYPES_ARRAY = config.get_current('_LTYPES_ARRAY')
...
def load_dmodel(model, from_user=False):
    ...
    """
    _DMODEL = libraries._get_DMODEL(model)
    ...
    # Sanitization of the model: priority
    vardic = {}
    toremove = []
    for k in dmodel['logics'].keys():
        v = dmodel['logics'][k]
        for key in v.keys():
...
def load_complete_DFIELDS(dmodel, verb=False):
    ...
    DFIELDS = copy.deepcopy(libraries._DFIELDS)
    ...
    for k0, v0 in dkout.items():
        for k1 in v0:
            ...
            # libraries._DFIELDS[k1]['eqtype'] = k0
            # libraries._DFIELDS[k1]['args'] = {key:[] for key in _LEQTYPES }
            # libraries._DFIELDS[k1]['kargs']= []

    # %% d) use dfields autocompletion
    DFIELDS2 = libraries._complete_DFIELDS(
        dfields=DFIELDS,
        complete=True,
        check=True,
    )
...
def add_numerical_group_default_fields(dparam, dfields):
    ...
    for k0, v0 in dparam.items():
        ...
        if 'eqtype' in v0.keys():
            if v0['eqtype'] == 'differential' and 'initial' not in v0.keys():
...
def extract_parameters(dparam, dfields, verb=None):
    ...
    if len(lpar_new + lfunc_new) > 0:
        ...
        for k0 in lpar_new + lfunc_new:
            key = k0
            if key not in libraries._DFIELDS.keys():
                dfail[k0] = "Unknown parameter"
                continue
            dparam[key] = dict(libraries._DFIELDS[key])
...
def _extract_par_from_func(lfunc=None, lpar=None, dparam=None, dfields=None):
    ...
    for k0 in lfunc:
        ...
        for kk in kargs:
            ...
            if key not in lkok:
                try:
                    if libraries._DFIELDS[key].get('func') is None:
...
def set_args_auxilliary(dparam, verb=False):
    ...
    for k0 in lfunc:
        ...
        dparam[k0]['args'] = {
            ...
            'parameter': [
                kk for kk in kargs
                if kk in lfunc and dparam[kk]['eqtype'] == 'parameter'
            ],
            'differential': [
                kk for kk in kargs
                if kk in lfunc and dparam[kk]['eqtype'] == 'differential'
            ],
            'statevar': [
                kk for kk in kargs
                if kk in lfunc and dparam[kk]['eqtype'] == 'statevar'
            ],
        }
...
def get_dargs_by_reference(dparam, dfunc_order):
    ...
    dargs = {
        k0: {
            k1: dparam[k1]['value']
            for k1 in (
                dparam[k0]['args']['differential'] + dparam[k0]['args']['statevar'] + dparam[k0]['args'][None]
            )
        }
        for k0 in dfunc_order['statevar'] + dfunc_order['differential']
    }
