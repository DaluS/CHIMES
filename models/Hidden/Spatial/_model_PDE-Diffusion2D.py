...
nablax=np.zeros((1,NN,NN,1));nablax[0,:,:,0]=diffmatX
nablay=np.zeros((1,NN,NN,1));nablay[0,:,:,0]=diffmatY*0
...
Lapx=np.zeros((1,NN,NN,1));Lapx[0,:,:,0]=lapX
Lapy=np.zeros((1,NN,NN,1));Lapy[0,:,:,0]=lapY

ZZ=np.exp(- 50*(np.sqrt((XX-0.5)**2+(YY-0.5)**2)))
C=np.zeros((1,NN,1,1));C[0,:,0,0]=ZZ.reshape(-1)
Y=np.zeros((1,NN,1,1));Y[0,:,0,0]=Y2
X=np.zeros((1,NN,1,1));X[0,:,0,0]=X2
