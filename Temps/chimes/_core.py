...
class Hub():
    ...
    * from_user : [Desactivated] BOOLEAN if you want to use your file (True) or the one provided in chimes (False)
    ...
    * verb : BOOLEAN True for print in terminal
    ...
    def get_presets(self,returnas=True):
        ...
        if returnas==True:
            return pd.DataFrame.from_dict({'Preset name' :[k for k in self.__dmodel['presets'].keys()],
                                           'Commentary':[v['com'] for v in self.__dmodel['presets'].values()] })
        else:
    ...
    def get_supplements(self,returnas=True):
        ...
        if returnas==dict:
            ...
            return pd.DataFrame.from_dict({'Supplement name' :[k for k in supp.keys()],
                                           'Commentary':[v.__doc__ for v in supp.values()] })
...
Npoints : TYPE, optional
    ...
    def get_equations_description(self,returnas = True):
        '''
        ...
        for key in self.__dmisc['dfunc_order']['differential']:
            v = self.dparam[key]      
            dic[key]={  'Type': 'Differential',
                        'Equations': f'd{key}/dt='+ v['source_exp'],
                        'Units': v['units'], 
                        'Definition':v['definition'],
                        'Comment': v['com'], 
                        'Dependency': ''.join([f"{key2}: {self.dparam[key2]['units']}, \
                                                 {self.dparam[key2]['units']}, \
                                                 {self.dparam[key2]['definition']} \n" for key2 in v['kargs'] ])}
        for key in self.__dmisc['dfunc_order']['statevar']:
            v = self.dparam[key]      
            dic[key]={  'Type': 'State Variable',
                        'Equations': f'{key}='+ v['source_exp'],
                        'Units': v['units'], 
                        'Definition':v['definition'],
                        'Comment': v['com'], 
                        'Dependency': ''.join([f"{key2}: {self.dparam[key2]['units']}, \
